...
// ========================================================================
// Only the contents *list* endpoint (constants.API_URL + "/contents?...")
// has local fallback data -- single-video-detail lookups
// (.../contents/{id}) still throw straight through on failure, same as
// before this fallback was added.
const isContentsListUrl = (url) => url.includes(`${constants.API_URL}/contents?`);
...
export const getVideosDataByUrl = async (url) => {
  try {
    ...
    if (!isContentsListUrl(url)) {
      throw error;
    }

    // External API unreachable (blocked, down, network error, etc) --
    // serve the local fallback video list instead so the app still shows
    // content rather than "No content available!".
    console.error("getVideosDataByUrl: external API failed, using local fallback:", error);
    const fallbackRes = await fetch("/api/fallback-contents");
