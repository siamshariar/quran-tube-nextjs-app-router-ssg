...
import PlayerModalTest from "./modal/PlayerModalTest";
...
export default function ContentPageTest({ getUrl, defaultMetaTitle, metaDescription, isDisplayLocalizationChipBar, isShorts }) {
    ...
    useEffect(() => {
        const initializeData = async () => {
            ...
            const initialUrl = getUrl({}, searchParam !== "" ? searchParam : initialActiveSubCat);
    ...
    useEffect(() => {
        if (isVisible && !isLoadingMore && data.pagination.c !== null) {
            setIsloadingMore(true);
            const url = getUrl(data.pagination, searchParam !== "" ? searchParam : activeSubCat);
            fetchData(url, true);
        }
    }, [isVisible]);
    ...
    return (
        <>
            ...
            <PlayerModalTest open={modalOpen} closer={handleModalClose} videoId={videoId} videoTitle={videoTitle} videoType={videoType} metaTitle={metaTitle} metaUrl={metaUrl} isIOS={isIOS} />
