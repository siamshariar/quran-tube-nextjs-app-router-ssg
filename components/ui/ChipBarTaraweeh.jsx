...
// remounts this component -- persisting the scroll offset here (outside
// React state) is what keeps the selected chip in view across that
// remount, instead of the list snapping back to the start ("All").
const SCROLL_STORAGE_KEY = "chipbar_taraweeh_scroll_left";
...
const ChipBarTaraweeh = ({ activeId, subCatClickHandler, pathname, taraweehPage }) => {
  ...
  const [scrollLeft, setScrollLeft] = useState(() => (taraweehPage === "taraweeh" ? 0 : getStoredScrollLeft()));
  ...
  useLayoutEffect(() => {
    ...
    if (taraweehPage !== "taraweeh") {
      storeScrollLeft(scrollLeft);
    }
  }, [scrollLeft, containerWidth, contentWidth, taraweehPage]);
    ...
    <div className={styles.wrapper}>
      <div className={styles.content} ref={containerRef} onScroll={handleNativeScroll}>
        <ul className={styles.list} ref={contentRef}>
          <Link href="/taraweeh" onClick={handleChipClick}>
            <li
          ...
          <Link href="/taraweeh-maqqa" onClick={handleChipClick}>
            <li
          ...
          <Link href="/taraweeh-madinah" onClick={handleChipClick}>
            <li
